...
class WaypointUpdater(object):
    ...
    def generate_lane(self):
        ...
        if self.stopline_wp_id == -1 or (self.stopline_wp_id >= farthest_id):
            lane.waypoints = base_waypoints
        else:
            lane.waypoints = self.decelerate_waypoints(
                base_waypoints, closest_id)

        return lane

    def decelerate_waypoints(self, waypoints, closest_id):
        result = []
        for i, wp in enumerate(waypoints):
            ...
            stop_id = max(self.stopline_wp_id - closest_id - 2, 0)
